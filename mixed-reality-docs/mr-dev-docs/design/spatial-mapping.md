...
The built-in NavMesh functionality in Unity cannot be used by default with spatial mapping surfaces. This is because spatial mapping surfaces aren't known until the application starts, you can install [NavmeshComponents](https://github.com/Unity-Technologies/NavMeshComponents) and use the components provided to build a navmesh during runtime. Also note that, the spatial mapping system won't provide [information about surfaces far away](spatial-mapping.md#the-environment-scanning-experience) from the user's current location. So the application must 'remember' surfaces itself if it's to build a map of a large area. You can also increase the observation extents in the spatial awareness profile to increase the area in which the navmesh can build.
